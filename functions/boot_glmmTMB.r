boot.glmmTMB<-function(model.res, data, excl.non.conv=F, nboots=1000, para=F, resol=100, level=0.95, use=NULL, contr=NULL, circ.var.name=NULL, circ.var=NULL, 
	...
	if(length(use)>0){
		...
		model.terms=c(attr(terms(as.formula(xcall[["cond.form"]])), "term.labels"), attr(terms(as.formula(xcall[["zi.form"]])), "term.labels"), attr(terms(as.formula(xcall[["zi.form"]])), "disp.form"))
		...
		if(xcall[["zi.form"]]!="~0"){
			...
			ci.fitted=ci.fitted*(1-ci.fitted.zi)
		}
		ci.fitted=data.frame(new.data, ci.fitted)
	}else{
		ci.fitted=NULL
	}
